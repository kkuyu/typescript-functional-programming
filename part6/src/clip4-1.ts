...
export const main = () => {
  const a = "abc";
  const b = f(a); // success(6)
  const c = T.flatMap(b, (b_) => g(b_)); // failed("6은 입력할 수 없습니다")
  const d = T.flatMap(c, (c_) => h(c_)); // failed("6은 입력할 수 없습니다")
  const result = T.map(d, (d_) => program(d_)); // failed("6은 입력할 수 없습니다")
